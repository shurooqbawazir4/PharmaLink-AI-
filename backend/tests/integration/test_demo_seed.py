...
from app.infrastructure.db.models import HospitalModel, InventoryModel, MedicineModel
from scripts import seed_database


async def test_demo_seed_is_repeatable(session_factory, monkeypatch, tmp_path):
    generator = runpy.run_path(str(Path(__file__).resolve().parents[3] / "data/synthetic/generate.py"))
    ...
    async with session_factory() as session:
        counts = [
            ...
            for model in (HospitalModel, MedicineModel, InventoryModel)
        ]
